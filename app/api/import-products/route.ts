...
export async function POST(req: Request) {
  try {
    ...
    // Преобразуем строки CSV в формат Product
    const productsData: Omit<
      Product,
      "id" | "createdAt" | "updatedAt" | "favorites" | "category" | "items"
    >[] = recordsRaw.map((row) => ({
      name: row.name,
      imageUrl: row.imageUrl || null,
      color: row.color || "",
      ...
      quantityPerPallet: row.quantityPerPallet
        ? parseFloat(row.quantityPerPallet)
        : null,
      palletsPerTruck: row.palletsPerTruck
        ? parseInt(row.palletsPerTruck)
        : null,
      totalPerTruck: row.totalPerTruck ? parseInt(row.totalPerTruck) : null,
      retailPriceRubWithVAT: row.retailPriceRubWithVAT
        ? parseFloat(row.retailPriceRubWithVAT)
        : null,
      city: row.city || null,
      categoryId: parseInt(row.categoryId),
      isOnSale: row.isOnSale === "true" || row.isOnSale === "1",
